    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li class="active"><?php
            ...
            echo $pageHeader;
            $sql_notify = "UPDATE tbl_notification 
                            SET checked_by = '$loginID', checked_time='$toDay', status='Checked'
                            WHERE order_id='$getSalesId' AND checked_by=0";
            $conn->query($sql_notify);
            $sql12 = "SELECT tbl_orders.id,tbl_orders.orderNo,tbl_orders.orderDate,tbl_orders.tbl_customerId,tbl_orders.tbl_transport_info,tbl_party.partyName,tbl_party.locationArea,tbl_party.tblCity 
                        FROM tbl_orders 
            ...
            while( $row = mysqli_fetch_array($result) ){
                ...
                $tbl_transportId= $row['tbl_transport_info'];
            }
          ...
          <div class="box">
            ...
            <div class="box-body">
				<form class="" id="form_salesReturn" method="POST" action="#">
					...
					<div class="form-group">
						<div class="col-sm-12">
							<label for="add_supplier">Transport : </label> 
							<select class="form-control" id="transportId" name="transportId"  style="width:100%;" disabled>';
							<?php
			                    $sql = "SELECT id,transportName,address FROM `tbl_transportInfo` WHERE id='$tbl_transportId' and status='Active'";
                                    $result = $conn->query($sql);
                                    while( $row = mysqli_fetch_array($result) ){
                                        echo '<option value="'.$row['id'].'">'.$row['transportName'].' - '.$row['address'].'</option>';
                                    }
                            ?>
							</select>
					...
					<div class="form-group">
						...
						<button type="button" class="btn btn-primary btn-flat" name="btn_salesReturn" id="btn_confirmOrder"><i class="fa fa-save"></i> Qty. Confirm & Next </button>
						<a href="orderList.php?page=Pending" class="btn btn-warning btn-flat"><i class="fa fa-mail-reply"></i> Back </a>
						<a class="btn btn-success btn-flat" id="RecalculationBtn"><i class="fa fa-calculator "></i> Recalculation </a>	
						</div>
					</div>
...
<script src="includes/js/checkOrderList.js"></script> 
