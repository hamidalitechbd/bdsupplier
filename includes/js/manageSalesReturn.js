	...
	$.ajax({
		...
		success: function(response){
			...
			for(var i=0;i<response.length;i++){
				...
				// End
				productHTML += "<tr id='productRow"+i+"'><td>"+input+""+(i+1)+"<input type='hidden' name='salesProductsId["+i+"]' value="+response[i].salesProductId+" /></td><td>"+response[i].productName+" - "+response[i].productCode+"</td><td>"+response[i].wareHouseName+"</td><td style='text-align: center;'><span id='quantity"+i+"' name='quantity["+i+"]'>"+response[i].quantity+"</td><td style='text-align: center;'><span id='returnedQuantity"+i+"' name='returnedQuantity["+i+"]'>"+response[i].returnedQuantity+"</span></td><td><input style='width: 100%;' type='text' id='returnQuantity"+i+"' name='returnQuantity["+i+"]' value='0'  onkeyup='calculateTotal("+i+")' "+disabled+" /></td><td style='text-align: center;'><span id='remainingQuantity"+i+"' name='remainingQuantity["+i+"]'>"+remainingQuantity+"</span></td><td>"+response[i].salesAmount+"</td><td>"+response[i].discount+"</td><td>"+response[i].grandTotal+"</td><td>"+showBtn+" <a href='#'  class='btn btn-danger btn-sm btn-flat' onclick='removeRows(" + i + ")'><i class='fa fa-trash'></i></a></td></tr>";
			}
