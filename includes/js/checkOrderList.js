...
$(document).ready(function() {
	orderSalesCheckTable = $("#orderSalesCheckTable").DataTable({
		'ajax': 'phpScripts/CheckOrderSalesAction.php?salesType=OrderSales&id='+$("#orderId").val(),
		'order': [],
		'dom': 'Bfrtip',
        'buttons': [
...
$("#btn_confirmOrder").click(function (){
    ...
    var wareHouseIds = [];
    i = 0;
    $('select[id^="warehouseId_"]').each(function() {
        if($(this).val() != ""){
            wareHouseIds[i] = $(this).val();
        }else{
            wareHouseIds[i] = "";
        }
        i = i + 1;
    ...
    fd.append('wareHouseIds',wareHouseIds);
    ...
    $.ajax({
        ...
        success: function(response){
            if(response == "Success"){
			    ...
			    location.href = 'confirmOrderWarehouse.php?salesId='+orderId+'&salesType=OrderSales';
            }else{
