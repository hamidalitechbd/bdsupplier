...
$(document).ready(function() {
	...
	manageSalesTable = $("#manageSalesTable").DataTable({
		'ajax': 'phpScripts/action.php?sortData='+$("#sortData").val(),
		'order': [],
		'dom': 'Bfrtip',
        'buttons': [
...
function loadCustomersSuppliers(tblType){
	...
	$.ajax({
        type: 'GET',
        url: 'phpScripts/loadWiCustomer.php',
        data: dataString,
        dataType: 'json',
        beforeSend: function () {
            $('#loading').show();
...
$("#sortData").change(function() {
    if($("#sortData").val() == "-1,-1"){
        if(checkLoadParty == 'No'){
            loadCustomersSuppliers("WI");
            $('#customer_div').append('<script>$("#customers").select2({placeholder: "~~ Select WI Phone Number ~~",allowClear: true,width: "100%"});</script>');
        }else{
        ...
        $("#customer_div").hide();
        //$("#customers").val("").trigger('change');
        manageSalesTable.ajax.url("phpScripts/action.php?sortData="+$("#sortData").val()).load();
        
    }
});
$("#customers").change(function() {
    if($("#sortData").val() == "-1,-1"){
        manageSalesTable.ajax.url("phpScripts/action.php?customerId="+$("#customers").val()).load();
        $("#customer_div").show();
    }
});
/*
